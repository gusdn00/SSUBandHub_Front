...
function TeamDetail() {
  ...
  // 지원자 목록 불러오기 (팀장일 경우만)
  useEffect(() => {
    if (loggedInUser?.uuid === team?.JoinTeams?.[0]?.user_id) {
      fetch(`http://localhost:3000/team/${teamId}/applicants`)
        .then(res => res.json())
        .then(data => setApplicants(data))
        .catch(err => console.error(err));
    }
  }, [team, teamId, loggedInUser]);
  ...
  //팀원 수락기능
    ...
    const res = await fetch(`http://localhost:3000/team/${teamId}/accept`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        user_id: userId,
        session_id: sessionId,
        leader_id: loggedInUser.uuid
      }),
    });
  ...
  const handleReject = async (userId) => {
    ...
    const res = await fetch(`http://localhost:3000/team/${teamId}/reject`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        user_id: userId,
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`http://localhost:3000/team/${team.id}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: loggedInUser?.uuid })
    });
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <p><strong>지역:</strong> {team.TeamRegion?.Region?.name || '미정'}</p>
      ...
      {/* 팀장일 경우: 삭제 버튼 & 지원자 목록 */}
      {loggedInUser?.uuid === team.JoinTeams?.[0]?.user_id && (
        <>
